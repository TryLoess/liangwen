...
def time_feature(df, name="time"):
    ...
    # close_weight：改为明确对比当前波动性与过去（前一日）20日波动均值
    ...
    return df, ["sin_time", "cos_time", 'weighted_price_change']#, "time_frac"] # + stage_dummies.columns.values.tolist()
...
def group_deal(df, group, name, window_length_list=[20], _type="ORI"):
...
def window_feature(df):
    window_length_list = [20]#, 10, 20, 40]
